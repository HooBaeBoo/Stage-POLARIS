...
*** 2018-06-25 Monday :
**** TASKS :
- [X] Appendleft When adding a task to ~To_explore~ indeed we do not append it but add it to head (append left)!
  - [X] Appendleft
  - [X] sorted
- Broadcast.c :
...
How to increase D? Values are very small, difficult to handle and the probability to make them match is pretty low.
First, let's correct how we append :
there are 2 differents cases : If the final state didn't meet any precomputed state then we just need to follow the policy to create a state to compute.
If a final state did meet a precomputed state then we need to be careful which state to compute in the future.
Basically, we have to test if the upper limit of the zone where my machine worked is now bigger or smaller than the current t.
If it is smaller then my paths, I will only add to ~To_explore~ paths above it and as usual the real state (t_curr , D_curr).
Otherwise I create a new state for that zone according to the policy chosen.
Lots of small mistakes were corrected :
Still, with the ~Generate_Dprime~, We need to make a random choice between border_inf border_sup. with integer and the use of random choice function these had to be different because you can't have a range between 0 and 0 for example.
No problem with floats. So we don't need to enlarge the space by 1!
We only leave in ~To_explore~ interessting values by checking at each round, if the computation is on a zone above t_curr.
And the output was made clearer by a better use Termcolor functions.
We'll add a variable to get more data on how efficient the process actually was if it was successful.
*** 2018-06-29 Friday :
[[http://simgrid.gforge.inria.fr/contrib/R_visualization.php][Source]]
R tutorial : rpy2
**** Installation :
pip3 install rpy2 :
*PROBLEM :* rpy2 is no longer supporting Python <3
*SOLUTION :* Install a older version 
#+begin_src shell :results output :exports both
pip3 install --user rpy2 == 2.8.6
#+end_src 
**** Instructions for use :
***** Import
First import package and everything it contains
#+begin_src shell :results output :exports both
import rpy2
from rpy2 import *
#+end_src
...
*** 2018-07-02 Monday :
**** Goal : higher D!
Increased NPROCS : increased D from 10^-3 to 10^-1
*WARNING* : If NPROCS is too big and the notebook not configured :
IOPub data rate exceeded.
#+begin_src shell :results output :exports both
jupyter notebook --NotebookApp.iopub_data_rate_limit=10000000  Time_Bcast.ipynb 
#+end_src
- [ ] Comment!
**** New method!
#+begin_src shell :results output :exports both
...
#+end_src
*** 2018-07-03 Tuesday :
apt-get install pajeng -> Marche pas
*SOLUTION* : Isolation cellule.
...
**** RGPD seminar : [[https://docs.google.com/document/d/10nxiXRDfCm4b1SjU0m_54UfeYhr2YD1f3_5Y7XfAIlM/edit?usp=sharing][here]]
**** Arnaud skype :
-> Do what Florence said with precision :)
*** 2018-07-10 Tuesday :
~state_match~ : test with the gap
*** 2018-07-11 Wednesday :
- smart borders : ajuster les bornes pour chaque processus à chaque tour. Si max est plus élevé je monte sinon je laisse. Idem pour min
- smart guess : moyenne de la valeur à chaque tour
- Repérer motif...? pattern learning
*** 2018-07-12 Thursday :
Cybersecurity day :)
Man in the middle attack / DNS Spoofing -> Big thanks to Alexandra for helping.
*** 2018-07-13 Friday :
Back to smart guess and smart borders!
For smart guess : I'll use a hash table : t-> average D.
Updating itself each round recomputing average value : ((average)(nb_val -1)+x)/nb_val
Related functions :
- ~reset_smart_table~
- ~update_smart_table~
